...
# You can change the training file below to any of the available training files.

#TRAINING_FILE = '/home/choh1/spliceCOV/src/liver.process_tiebrush.processed_round1.bundles_with_labels.train.txt'
# TRAINING_FILE = '/home/choh1/spliceCOV/src/small_intestine.processtiebrush_round1.bundles_with_labels.txt'
#TRAINING_FILE = '/ccb/salz1/choh1/spliceCov/scripts/model_output/blood.process_tiebrush.processed_round1.bundles_with_labels.txt'
# TRAINING_FILE = '/home/choh1/spliceCOV/src/bone_marrow.process_tiebrush.processed_round1.bundles_with_labels.txt'
...
OUTPUT_DIR = '/ccb/salz1/choh1/spliceCov/scripts/model_output/'  # Define the output directory for encoders
...
def train_model(print_row_index=0, scaling_factor=1.0):
    ...
    columns_training = [
        ...
        'position',         # $2
        'junction_id',      # $3
        'num_samples',      # $4 - We'll normalize by median, then scale
        'strand',           # $5
        'perc',             # $6 - Binarize
        'cov_diff',         # $7 
        'perc_cov_diff',    # $8
        'junc_len',         # $9 - We'll log-transform
        'smooth_metric',    # $10 - We'll standard scale this
    ...
    # NOTE: In a previous version, you created an extra feature "cov_diff_norm" here.
    # To match the number of features in the saved model (7 features), we remove that step.

    # 3) Log-transform 'junc_len'
    print("Log-transforming 'junc_len'...")
    X['junc_len_log'] = np.log1p(X['junc_len'])  # log1p safely handles junc_len=0
    X = X.drop('junc_len', axis=1)

    # 4) Standard scale 'smooth_metric'
    print("Standard scaling 'smooth_metric'...")
    smooth_metric_mean = X['smooth_metric'].mean()
    smooth_metric_std = X['smooth_metric'].std()
    if smooth_metric_std == 0:
        print("Warning: 'smooth_metric' has zero standard deviation. Skipping standard scaling.")
        X['smooth_metric_scaled'] = 0.0
    else:
        X['smooth_metric_scaled'] = (X['smooth_metric'] - smooth_metric_mean) / smooth_metric_std
    X = X.drop('smooth_metric', axis=1)

    # At this point the feature columns (order matters) are:
    # ['cov_diff', 'perc_cov_diff', 'cov_change_dir', 'perc_binarized', 'num_samples_scaled', 'junc_len_log', 'smooth_metric_scaled']
    ...
    encoders = {
        ...
        'smooth_metric_mean': smooth_metric_mean,
        'smooth_metric_std': smooth_metric_std,
...
def score_data(testing_file, output_file):
    ...
    # NOTE: As in training, we do not add an extra "cov_diff_norm" column.

    # 3) Log-transform 'junc_len'
    print("Log-transforming 'junc_len'...")
    X['junc_len_log'] = np.log1p(X['junc_len'])
    X = X.drop('junc_len', axis=1)

    # 4) Standard scale 'smooth_metric'
    print("Standard scaling 'smooth_metric'...")
    smooth_metric_mean = encoders.get('smooth_metric_mean', 0.0)
    smooth_metric_std = encoders.get('smooth_metric_std', 1.0)
    if smooth_metric_std == 0:
        print("Warning: 'smooth_metric' has zero standard deviation. Skipping standard scaling.")
        X['smooth_metric_scaled'] = 0.0
    else:
    ...
    # At this point the feature columns should be in the same order as during training:
    # ['cov_diff', 'perc_cov_diff', 'cov_change_dir', 'perc_binarized', 'num_samples_scaled', 'junc_len_log', 'smooth_metric_scaled']
    ...
    smooth_override_mask = (smooth_metric_raw < 10) & (data['perc_cov_diff'] <= 0.25) & (data['junc_len'] <= 15)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--scale', dest='scaling_factor', type=float, default=1.0,
                        help='Scaling factor to normalize num_samples and cov_diff by dividing them by this number (default: 1.0).')
